...
const HIGH_RISK_TLDS = new Set(["zip", "mov", "top", "click", "gq", "cf", "tk", "ml"]);

const SECURITY_RULES = [
  (url) => ["http:", "https:"].includes(url.protocol),

  (url) => !["localhost", "metadata.google.internal", "[::1]"].includes(url.hostname.toLowerCase()),

  (url) => {
    const hostname = url.hostname.toLowerCase();
    const parts = hostname.split(".");
    
    if (parts.length === 4 && parts.every(part => !isNaN(part) && !isNaN(parseFloat(part)))) {
      try {
    ...
    if (parts.length === 1 && !isNaN(hostname) && !isNaN(parseFloat(hostname))) {
      return false;
    }
